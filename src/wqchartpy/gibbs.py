...
def plot(df, 
    ...
    # Creat the legend
    n_legend = math.ceil(len(set(df['Label'])) / 8)
    ax1.legend(
        loc='upper left', 
        bbox_to_anchor=(0.02, 0.98), # Coordenadas dentro del gráfico (x, y)            borderaxespad=0.,
        ncol = n_legend,
        frameon=False,
        fontsize=10
    )
    ...
    return ax1 # before was [ax1, ax2] but both are atach to the same figure so they end being both the same (containgn both plots)
